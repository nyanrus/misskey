...
// Cache for Web Crypto CryptoKey objects keyed by PEM string.
// Avoids repeated PKCS#8 import on every sign operation.
type WebCryptoKey = Awaited<ReturnType<typeof crypto.subtle.importKey>>;
const cryptoKeyCache = new Map<string, WebCryptoKey>();
const textEncoder = new TextEncoder();

async function importSigningKey(pem: string): Promise<WebCryptoKey> {
	const cached = cryptoKeyCache.get(pem);
...
export class ApRequestCreator {
	...
	static async #signToRequest(request: Request, key: PrivateKey, includeHeaders: string[]): Promise<Signed> {
		...
		const cryptoKey = await importSigningKey(key.privateKeyPem);
		const signatureBuffer = await crypto.subtle.sign('RSASSA-PKCS1-v1_5', cryptoKey, textEncoder.encode(signingString));
